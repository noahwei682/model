...
deepspeed --num_gpus=8 llava/train/train_mem.py \
    ...
    --version v1 \
    --data_path $FINE_DATA \
    --text_embedding_path $FINE_EMB \
    --image_folder $FINE_IMAGE \
    --vision_tower EVA02-CLIP-L-14-336  \
    --eva_ckpt_path $EVA_CKPT \
    --run_name llava-v1.5-7b-$CKPT_DIR \
    --pretrain_mm_mlp_adapter $MLP_CKPT_PREFIX-$CKPT_DIR-pretrain/mm_projector.bin \
    ...
    --bf16 True \
    --output_dir $MODEL_CKPT_DIR-$CKPT_DIR \
    --num_train_epochs 1 \
